...
**Respuesta: -data mining -> entender a partir de datos desordenados , ML -> predecir a partir de problemas especificos**
...
**Respuesta: predict: a partir de caracteristicas de los datos se pueden hacer inferencias sobre futuros valores que se quieran estudiar, mientras que los descriptivos buscan patrones que describan los datos. Clasif predict, Clustering descript. **

*3. ¿Qué es el análisis exploratorio de datos o EDA?*

**Respuesta: Es la primera aproximacion que se hace para estudiar los datos, se calculan distintos estadisticos de forma que la info contenida pueda ser interpretada mas facilmente. Por ejemplo calcular la media, desviacion estandar, ver que tipo de datos se tiene, ver si los datos distribuyen de forma especifica, etc **

*4. Explique cómo se identifican los valores atípicos u outliers en un boxplot y cuál es su utilidad.*

**Respuesta: El diagrama de caja tiene demarcados los cuartiles de la muestra, los outliers estan representados fuera del rango de este boxplot (las pestañas que salen de las cajas). El boxplot, muestra la mediana y los demas cuartiles, al representar los cuartiles permite visualizar rapidamente como estan distribuidos (donde se concentran los datos, el RIC) los datos y ver cuando los datos son outliers.**
...
```
**Respuesta: Cada linea en el dataset representa la frecuencia con que cada concepto constitucional (columnas) fue mencionado en la localidad indicada. Así, tomando como ejemplo la fila 45 del dataset tenemos que el concepto 'a huelga' fue mencionado una vez  **
